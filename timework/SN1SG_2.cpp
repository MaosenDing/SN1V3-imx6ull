...
struct stu1 SolarPos(int Year, int Month, int Day, int Hour, int Minute, int Second, int Delta_T, double Longitude, double Latitude, int E, int P, int T)
{
	...
	double L0_T[64][4] = { { 0, 175347046, 0, 0 }, { 1, 3341656, 4.6692568, 6283.07585 }, { 2, 34894, 4.6261, 12566.1517 }, \
	...
	double L1_T[34][4] = { { 0, 628331966747, 0, 0 }, { 1, 206059, 2.678235, 6283.07585 }, { 2, 4303, 2.6351, 12566.1517 }, \
	...
	double L2_T[20][4] = { { 0, 52919, 0, 0 }, { 1, 8720, 1.0721, 6283.0758 }, { 2, 309, 0.867, 12566.152 }, { 3, 27, 0.05, 3.52 }, \
	...
	double L3_T[7][4] = { { 0, 289, 5.844, 6283.076 }, { 1, 35, 0, 0 }, { 2, 17, 5.49, 12566.15 }, { 3, 3, 5.2, 155.42 }, \
	{ 4, 1, 4.72, 3.52 }, { 5, 1, 5.3, 18849.23 }, { 6, 1, 5.97, 242.73 } };

	double L4_T[3][4] = { { 0, 114, 3.142, 0 }, { 1, 8, 4.13, 6283.08 }, { 2, 1, 3.84, 12566.15 } };

	double L5_T[1][4] = { { 0, 1, 3.14, 0 } };

	double B0_T[5][4] = { { 0, 280, 3.199, 84334.662 }, { 1, 102, 5.422, 5507.553 }, { 2, 80, 3.88, 5223.69 }, \
	{ 3, 44, 3.7, 2352.87 }, { 4, 32, 4, 1577.34 } };

	double B1_T[2][4] = { { 0, 9, 3.9, 5507.55 }, { 1, 6, 1.73, 5223.69 } };

	double R0_T[40][4] = { { 0, 100013989, 0, 0 }, { 1, 1670700, 3.0984635, 6283.07585 }, { 2, 13956, 3.05525, 12566.1517 }, \
	...
	double R1_T[10][4] = { { 0, 103019, 1.10749, 6283.07585 }, { 1, 1721, 1.0644, 12566.1517 }, { 2, 702, 3.142, 0 }, \
	...
	double R2_T[6][4] = { { 0, 4359, 5.7846, 6283.0758 }, { 1, 124, 5.579, 12566.152 }, { 2, 12, 3.14, 0 }, \
	{ 3, 9, 3.63, 77713.77 }, { 4, 6, 1.87, 5573.14 }, { 5, 3, 5.47, 18849.23 } };

	double R3_T[2][4] = { { 0, 145, 4.273, 6283.076 }, { 1, 7, 3.92, 12566.15 } };

	double R4_T[1][4] = { { 0, 4, 2.56, 6283.08 } };
...
#define sheetfile stdout
	...
	struct stu1 tempSP;
	double ZxAng, YxAng, sinY, cosY, sinZ, cosZ, temp1, temp2;
	int Hour2, Minute2, Second2;
	int Hour, Minute, Second;
	int flag = 0;
	double tempV[3], tempN[3], tempn;
	for (Hour = 4; Hour <= 20; Hour++)
	{
		for (Minute = 0; Minute < 60; Minute++)
		{
			for (Second = 0; Second < 60; Second = Second + HelioAdjTime)
			{
				...
				temp1 = tempV[1] * cos(EP5) - tan(EP5)*(tempN[2] - tempV[1] * sin(EP5));
				temp2 = tempV[0] * cosY + tempV[2]*sinY;
				
				sinZ = (tempN[1] * temp2 - tempN[0] * temp1) / (pow(temp1, 2) + pow(temp2, 2));
				cosZ = (tempN[0] * temp2 + tempN[1] * temp1) / (pow(temp1, 2) + pow(temp2, 2));
